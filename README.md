...
The training data is synthetic and generated by `build_manipulation_dataset.py`. The data generation is intentionally simple: it creates balanced examples across manipulation labels, intents, tactics, and domains so the model can learn structured outputs consistently.
...
- Text feature matrix shape after vectorization: `480 x 9775`

Distribution:

- Labels: `240 manipulative`, `240 not_manipulative`
- Domains: `10`, each with `48` rows
- Intent classes: `12`, each with `40` rows
...
| `intent_label` | `0.9646` | `0.9645` | `0.9645` |
| `manipulation_type` | `1.0000` | `1.0000` | `1.0000` |
| `domain` | `0.9875` | `0.9875` | `0.9875` |
...
Important note: these results are strong because the project currently evaluates on synthetic folds drawn from the same data generation regime. If you want production-level confidence, the next step is evaluation on a separate external dataset or newly written human-authored examples.
